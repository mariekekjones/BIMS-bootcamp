...
#### \*\* EXERCISE 1 ** ** YOUR TURN \*\* A. How many observations are there of children (&lt; 18 years old)? B. How many cases of obese children are there (BMI &gt;= 30)? C. Use `filter()`, `group_by()` and `summarize()` to find the mean BMI by Smoking Status for only Adults who have Diabetes. Do smokers or non-smokers have higher BMI?
...
# so add some random variability to x-axis
nh %>%
...
A. ![No = not physically active, Yes = physically active](plots/Plot4a.png)

B. ![Custom colors are "salmon" and "seagreen"](plots/Plot4b.png)
