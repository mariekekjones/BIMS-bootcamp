...
C. Use `filter()`, `group_by()` and `summarize()` to find the mean BMI by Smoking Status for only Adults who have Diabetes. Do smokers or non-smokers have higher BMI?
...
p <- nh %>%
  filter(!is.na(SmokingStatus)) %>%
  ggplot(aes(x = SmokingStatus, y = BMI, fill = Diabetes)) + 
  geom_boxplot()
p
...
### ** EXERCISE 3 ** --------------
** YOUR TURN **
A. Create a plot showing the association between RelationshipStatus and the Number of Alcoholic drinks yearly (`AlcoholYear`) for adults.

B. Choose your own colors to show the effect of Gender on this association.
```{r}
#A
nh %>%
  filter(Age > 18) %>%
